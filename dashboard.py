...
# ── Tabs ──────────────────────────────────────────────────────────────────
tab1, tab2, tab3, tab4, tab5 = st.tabs(["🗺️ District Analysis", "📉 Uptake Gap", "🔬 Statistical Analysis", "💡 Interventions", "🗺️ India Map"])
...
# ── TAB 5: India Map ──────────────────────────────────────────────────────
with tab5:
    st.subheader("🗺️ India Choropleth — PM-KISAN Uptake by State")
    st.markdown("Red dots = Bottom 10 priority districts. Click on them for details.")

    with open("outputs/india_pmkisan_map.html", "r", encoding="utf-8") as f:
        map_html = f.read()
    st.components.v1.html(map_html, height=500, scrolling=True)

    st.markdown("---")
    st.subheader("👩‍🌾 Gender & Demographic Disparity Analysis")

    col1, col2 = st.columns(2)

    with col1:
        fig_gender = px.scatter(df,
            x='Female_Farmer_Pct',
            y='Uptake_Pct',
            color='State',
            size='Gap_Farmers',
            hover_data=['District'],
            title='Female Farmer % vs PM-KISAN Uptake',
            labels={'Female_Farmer_Pct': 'Female Farmer %', 'Uptake_Pct': 'Uptake (%)'}
        )
        r_g, p_g = stats.pearsonr(df['Female_Farmer_Pct'], df['Uptake_Pct'])
        fig_gender.add_annotation(
            text=f"r = {r_g:.2f}, p = {p_g:.3f}",
    ...
    with col2:
        fig_land = px.scatter(df,
            x='Avg_Landholding_Acres',
            y='Uptake_Pct',
            color='State',
            size='Gap_Farmers',
            hover_data=['District'],
            title='Avg Landholding vs PM-KISAN Uptake',
            labels={'Avg_Landholding_Acres': 'Avg Landholding (Acres)', 'Uptake_Pct': 'Uptake (%)'}
        )
        r_l, p_l = stats.pearsonr(df['Avg_Landholding_Acres'], df['Uptake_Pct'])
        fig_land.add_annotation(
            text=f"r = {r_l:.2f}, p = {p_l:.3f}",
            xref="paper", yref="paper", x=0.05, y=0.95,
            showarrow=False,
            font=dict(size=13, color="red")
        )
    ...
    st.subheader("📊 District Segmentation Model")
    df['Risk_Category'] = pd.cut(df['Uptake_Pct'],
        bins=[0, 30, 45, 60, 100],
        labels=['🔴 Critical (<30%)', '🟠 High Risk (30-45%)', '🟡 Moderate (45-60%)', '🟢 Good (>60%)']
    )
    seg_counts = df['Risk_Category'].value_counts().reset_index()
    seg_counts.columns = ['Category', 'Count']
    fig_seg = px.pie(seg_counts, values='Count', names='Category',
                     title='District Segmentation by Uptake Risk Level',
                     color_discrete_map={
                         '🔴 Critical (<30%)': '#e74c3c',
                         '🟠 High Risk (30-45%)': '#e67e22',
                         '🟡 Moderate (45-60%)': '#f1c40f',
                         '🟢 Good (>60%)': '#2ecc71'
                     })
    st.plotly_chart(fig_seg, use_container_width=True)

    st.dataframe(
        df[['District','State','Uptake_Pct','Risk_Category','Gap_Farmers']]
        .sort_values('Uptake_Pct')
        .reset_index(drop=True),
        use_container_width=True
    )
